...
export async function confirmTicketPurchase(verificationToken: string): Promise<void> {
  const res = await fetch(`${BASE}/api/v1/event/tickets/confirm-purchase`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => ({ message: res.statusText })) as { message?: string }
    throw new Error(body.message ?? 'Failed to confirm purchase')
  }
}
