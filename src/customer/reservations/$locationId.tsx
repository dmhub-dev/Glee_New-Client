...
import { useCreateReservation, useReservationAvailability, useReservationVenue } from '@glee/api'
import { Badge, Button, Input, Skeleton, useToast } from '@glee/ui'
import { Calendar, Clock, MapPin, ShieldCheck, Users, Wallet } from 'lucide-react'
...
export default function CustomerReservationVenuePage() {
  ...
  async function handleReserve() {
    ...
    try {
      ...
      navigate(`/app/reservations/detail/${reservation.id}`)
    } catch (error) {
      toast({
    ...
    return (
      <CustomerLayout title="Reservation" hidePageHeader>
        <div className="mx-auto w-full max-w-3xl px-4 py-12 text-center">
          ...
          <Button onClick={() => navigate('/app/reservations')} className="mt-4 rounded-full bg-neon-pink text-white hover:bg-neon-pink/90">
  ...
  return (
    <CustomerLayout title={venue.name} hidePageHeader>
      <div className="mx-auto w-full max-w-7xl space-y-4 px-4 pb-32 pt-5 lg:px-8">
        ...
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <aside className="h-fit rounded-3xl bg-white/[0.08] p-5 shadow-[0_18px_55px_rgba(0,0,0,0.22)]">
            ...
            <h2 className="mt-3 font-heading text-2xl font-black text-white">Reserve with wallet</h2>
            ...
            >
              ...
              {createReservation.isPending ? 'Reserving...' : 'Pay Deposit With Wallet'}
            </Button>
          </aside>
        </section>
      </div>
