...
const eventSchema = z.object({
  ...
  status: z.enum(['draft', 'pending_approval', 'active', 'live', 'ended', 'postponed', 'cancelled', 'rejected', 'sold_out'] as const),
  startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Use YYYY-MM-DD format'),
  endDate:   z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Use YYYY-MM-DD format'),
  startTime: z.string().regex(/^\d{2}:\d{2}$/, 'Use HH:MM format'),
...
const STATUS_OPTIONS: { value: EventFormValues['status']; label: string; description: string; className: string }[] = [
  ...
  { value: 'live',             label: 'Live',             description: 'Check-in is running',       className: 'border-pink-500/30 bg-pink-500/10 text-pink-400' },
  { value: 'ended',            label: 'Ended',            description: 'Event has finished',        className: 'border-admin bg-admin-overlay text-admin-50' },
