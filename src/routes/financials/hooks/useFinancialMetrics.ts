...
async function getFinancialsOverview(range: TimeRange, upcomingLimit: number, recentSalesLimit: number) {
  const res = await apiFetch<unknown>(
    `/api/v1/finance/reports/overview?range=${range}&upcomingLimit=${upcomingLimit}&recentSalesLimit=${recentSalesLimit}`,
  )
  return unwrapData<Record<string, unknown>>(res) ?? {}
}

async function getFinancialsRecentSales(limit: number) {
  const res = await apiFetch<unknown>(`/api/v1/finance/reports/recent-sales?limit=${limit}`)
...
async function getFinancialsRevenueByRange(range: TimeRange) {
  const res = await apiFetch<unknown>(`/api/v1/finance/reports/revenue?range=${range}`)
...
async function getFinancialsDailyEarnings(range: TimeRange) {
  const res = await apiFetch<unknown>(`/api/v1/finance/reports/daily-earnings?range=${range}`)
  const data = unwrapData<unknown>(res)
...
async function getFinancialsTicketRevenue(range: TimeRange): Promise<number> {
  const res = await apiFetch<unknown>(`/api/v1/finance/reports/ticket-revenue?range=${range}`)
  const data = unwrapData<any>(res)
...
async function getFinancialsHighestSellingEvent(range: TimeRange): Promise<HighestSellingEvent | null> {
  const res = await apiFetch<unknown>(`/api/v1/finance/reports/highest-selling-event?range=${range}`)
  const data = unwrapData<any>(res)
  if (!data) return null
...
async function getFinancialsRecentPayouts(limit: number) {
  const res = await apiFetch<unknown>(`/api/v1/finance/reports/recent-payouts?limit=${limit}`)
  const data = unwrapData<unknown>(res)
...
async function getFinancialsMonthlyTrend(months = 12): Promise<MonthlyTrendRow[]> {
  const res = await apiFetch<unknown>(`/api/v1/finance/reports/monthly-trend?months=${months}`)
  const data = unwrapData<any>(res)
  const container = data?.data !== undefined ? data.data : data
