...
test('App registers payouts route and removes vendor location route access', async () => {
  ...
  assert.match(source, /const\s+LOCATION_ROLES:\s*UserRole\[\]\s*=\s*\[\.\.\.ADMIN_ROLES\]/)
...
test('vendor location section is reference-only', async () => {
  const source = await readFile(new URL('../src/routes/events/index.tsx', import.meta.url), 'utf8')
  assertTranspiles(source)
  assert.match(source, /Use Glee-approved locations for your events/)
  assert.doesNotMatch(source, /<LocationReferenceGrid\s+locations=\{locations\s*\?\?\s*\[\]\}\s+isLoading=\{locationsLoading\}\s+allowCreate\s*\/>/)
  assert.match(source, /<LocationReferenceGrid\s+locations=\{locations\s*\?\?\s*\[\]\}\s+isLoading=\{locationsLoading\}\s*\/>/)
})
